...
@interface ViewController () <SystemCaptureManagerDelegate,AudioEncoderDelegate,AudioDecoderDelegate>
...
- (void)viewDidLoad {
    ...
    // 3.音频解码器
    self.audioDecoder = [[AudioDecoder alloc] initWithConfig:[AudioConfig defaulConfig]];
    self.audioDecoder.delegate = self;
    
    // 4.PCM播放器
    self.pcmPlayer = [[AudioPCMPlayer alloc] initWithConfig:[AudioConfig defaulConfig]];
}
...
#pragma mark - AudioDecoder Delegate
- (void)audioDecodeCallback:(NSData *)pcmData {
    if(pcmData) {
        size_t writeLength = fwrite(pcmData.bytes, 1, pcmData.length, self.pcm_file);
        if (writeLength != pcmData.length) {
            NSLog(@"write pcm data error \n");
        }
    }
