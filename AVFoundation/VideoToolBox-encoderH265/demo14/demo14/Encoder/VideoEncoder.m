...
@property (nonatomic, copy) NSMutableData *vpsData;
...
const uint8_t startCode4[] = "\x00\x00\x00\x01";
...
- (instancetype)initWithConfig:(VideoConfig *)config
{
    ...
    if (self) {
        ...
        CMVideoCodecType videoCodecType = [self _deviceSupportH265Encode] ? kCMVideoCodecType_HEVC : kCMVideoCodecType_H264;
        
        //创建编码会话
        OSStatus status = VTCompressionSessionCreate(kCFAllocatorDefault, (int32_t)_videoConfig.width, (int32_t)_videoConfig.height, videoCodecType, NULL, NULL, NULL, VideoEncodeCallback, (__bridge void * _Nullable)(self), &_vtSession);
        if (status != noErr) {
