...
const NSString * fragment_shader = @"                   \
...
-(void)renderLayer
{
    ...
    glViewport(self.frame.origin.x * scale, self.frame.origin.y * scale, width, height);
    
    //2.读取顶点着色程序、片元着色程序
    
    //3.加载shader
    self.myPrograme = [self loadShaders:nil Withfrag:nil];
...
- (GLuint)setupTexture2:(NSString *)fileName {
    NSError *error ;
    GLKTextureInfo *info = [GLKTextureLoader textureWithCGImage:[UIImage imageNamed:fileName].CGImage options:nil error:&error];
    if (error) {
        NSLog(@"create TextureInfo error ");
    }
    return info.name;
}
...
-(GLuint)loadShaders:(NSString *)vert Withfrag:(NSString *)frag
{
    ...
    //参数3：文件路径
    [self compileShader:&verShader type:GL_VERTEX_SHADER content:vertex_shader];
    [self compileShader:&fragShader type:GL_FRAGMENT_SHADER content:fragment_shader];
