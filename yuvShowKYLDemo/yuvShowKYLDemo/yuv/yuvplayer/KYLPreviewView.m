...
#define kModuleName "KYLPreviewView"
enum
{
    KYL_UNIFORM_Y,
    KYL_UNIFORM_UV,
    KYL_UNIFORM_COLOR_CONVERSION_MATRIX,
    KYL_NUM_UNIFORMS
};
...
GLfloat kKYLPreViewColorConversion601FullRange[] = {
    1.0,    1.0,    1.0,
    0.0,    -0.343, 1.765,
    1.4,    -0.711, 0.0,
};
...
GLint uniforms11[KYL_NUM_UNIFORMS];

@interface KYLPreviewView ()
{
    GLint _backingWidth;
...
@property (nonatomic, assign) KYLPixelBufferType bufferType;
@property (nonatomic, assign) KYLPixelBufferType lastBufferType;
...
- (void)initPreview {
    ...
    self.screenWidth = 0;
    self.bufferType = KYLPixelBufferTypeNV12;
    self.lastBufferType = KYLPixelBufferTypeNone;
    _preferredConversion = kKYLPreViewColorConversion601FullRange;
...
- (void)displayPixelBuffer:(CVPixelBufferRef)pixelBuffer videoTextureCache:(CVOpenGLESTextureCacheRef)videoTextureCache context:(EAGLContext *)context backingWidth:(GLint)backingWidth backingHeight:(GLint)backingHeight frameBufferHandle:(GLuint)frameBufferHandle nv12Program:(GLuint)nv12Program rgbProgram:(GLuint)rgbProgram preferredConversion:(const GLfloat *)preferredConversion displayInputTextureUniform:(GLuint)displayInputTextureUniform colorBufferHandle:(GLuint)colorBufferHandle{
    ...
    if (bufferType == KYLPixelBufferTypeNV12) {
        if (self.lastBufferType != bufferType) {
            ...
            glUniform1i(uniforms11[KYL_UNIFORM_Y], 0);
            glUniform1i(uniforms11[KYL_UNIFORM_UV], 1);
            glUniformMatrix3fv(uniforms11[KYL_UNIFORM_COLOR_CONVERSION_MATRIX], 1, GL_FALSE, preferredConversion);
        }
    } else if (bufferType == KYLPixelBufferTypeRGB) {
        if (self.lastBufferType != bufferType) {
    ...
    glVertexAttribPointer(KYL_ATTRIB_VERTEX, 2, GL_FLOAT, 0, 0, quadVertexData);
    glEnableVertexAttribArray(KYL_ATTRIB_VERTEX);
    
    glVertexAttribPointer(KYL_ATTRIB_TEXCOORD, 2, GL_FLOAT, 0, 0, quadTextureData);
    glEnableVertexAttribArray(KYL_ATTRIB_TEXCOORD);
...
- (EAGLContext *)createOpenGLContextWithWidth:(int *)width height:(int *)height videoTextureCache:(CVOpenGLESTextureCacheRef *)videoTextureCache colorBufferHandle:(GLuint *)colorBufferHandle frameBufferHandle:(GLuint *)frameBufferHandle {
    ...
    [self loadShaderWithBufferType:KYLPixelBufferTypeNV12];
    [self loadShaderWithBufferType:KYLPixelBufferTypeRGB];
...
- (void)setupBuffersWithContext:(EAGLContext *)context width:(int *)width height:(int *)height colorBufferHandle:(GLuint *)colorBufferHandle frameBufferHandle:(GLuint *)frameBufferHandle {
    ...
    glEnableVertexAttribArray(KYL_ATTRIB_VERTEX);
    glVertexAttribPointer(KYL_ATTRIB_VERTEX, 2, GL_FLOAT, GL_FALSE, 2 * sizeof(GLfloat), 0);
    
    glEnableVertexAttribArray(KYL_ATTRIB_TEXCOORD);
    glVertexAttribPointer(KYL_ATTRIB_TEXCOORD, 2, GL_FLOAT, GL_FALSE, 2 * sizeof(GLfloat), 0);
...
- (void)loadShaderWithBufferType:(KYLPixelBufferType)type {
    ...
    glBindAttribLocation(program, KYL_ATTRIB_VERTEX  , "position");
    glBindAttribLocation(program, KYL_ATTRIB_TEXCOORD, "inputTextureCoordinate");
    ...
    if (type == KYLPixelBufferTypeNV12) {
        uniforms11[KYL_UNIFORM_Y] = glGetUniformLocation(program , "luminanceTexture");
        uniforms11[KYL_UNIFORM_UV] = glGetUniformLocation(program, "chrominanceTexture");
        uniforms11[KYL_UNIFORM_COLOR_CONVERSION_MATRIX] = glGetUniformLocation(program, "colorConversionMatrix");
    } else if (type == KYLPixelBufferTypeRGB) {
