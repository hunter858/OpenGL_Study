...
-(void)setup:(Callback)callback{
    ...
    self.previewLayer.bounds = CGRectMake(0, 0, self.bounds.size.width, self.bounds.size.height);
...
-(void)captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection{
    ...
    NSInteger rows = CVPixelBufferGetBytesPerRowOfPlane(imageBuffer,0);
    void* lumaBuffer = CVPixelBufferGetBaseAddressOfPlane (imageBuffer,0);

    
    CGColorSpaceRef colorSpace = CGColorSpaceCreateDeviceGray();
    CGContextRef context = CGBitmapContextCreate(lumaBuffer, width, height, 8, rows, colorSpace,0);
    CGImageRef imageRef =  CGBitmapContextCreateImage(context);
    dispatch_async(dispatch_get_main_queue(), ^{
        self.callback(imageRef);
