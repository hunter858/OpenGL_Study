...
@property (nonatomic,assign) ChangeDirection currentDirection;
...
-(instancetype)init {
    self = [super init];
    if (self) {
        self.currentDirection = ChangeDirectionZ;
    }
    return self;
}
...
- (void)render {
    ...
    [self rotateDirctWithValue:self.value];
...
- (void)setX:(CGFloat)x {
    self.currentDirection = ChangeDirectionX;
    self.value = x;
    [self render];
}

-(void)setY:(CGFloat)y {
    self.currentDirection = ChangeDirectionY;
    self.value = y;
    [self render];
}
...
- (void)rotateDirctWithValue:(CGFloat)value {
    switch (self.currentDirection) {
        case ChangeDirectionX:
            [self transformWithX:value];
            break;
        case ChangeDirectionY:
            [self transformWithY:value];
            break;
        case ChangeDirectionZ:
            [self transformWithZ:value];
            break;
        default:
            self.currentDirection = ChangeDirectionZ;
            break;
...
- (void)transformWithX:(CGFloat )angle {
    ...
    //设置旋转矩阵
    glUniformMatrix4fv(rotate, 1, GL_FALSE, (GLfloat *)&xRotation[0]);
}

-(void)transformWithY:(CGFloat )angle {
    GLuint rotate = glGetUniformLocation(self.mProgram, "rotateMatrix");
    ...
    //设置旋转矩阵
    glUniformMatrix4fv(rotate, 1, GL_FALSE, (GLfloat *)&yRotation);
    
}

- (void)transformWithZ:(CGFloat )angle {
    GLuint rotate = glGetUniformLocation(self.mProgram, "rotateMatrix");
